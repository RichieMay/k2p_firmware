...
-- @async true/false, execute async or sync
function Cursor.apply(self, configlist, command, async)
	configlist = self:_affected(configlist)
	if command then
		return { "/sbin/luci-reload", unpack(configlist) }
	else
		local cmd = "/sbin/luci-reload %s >/dev/null 2>&1" % table.concat(configlist, " ")
		if async then
