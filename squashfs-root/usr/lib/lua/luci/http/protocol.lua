...
--Adjust 500KB for gfwlist
HTTP_MAX_CONTENT      = 1024*500		-- 500 KB maximum content size
...
function urldecode_message_body( src, msg )
	...
	local ret, err = ltn12.pump.all( src, snk )
...
--- Decode an json http message body with application/json Content-Type.
...
function jsondecode_message_body( src, msg )
    ...
    local json_msg = jsonlib.decode(data, urldecode)
    if not json_msg then
        return nil, "Message body is not legal json string" 
    end
