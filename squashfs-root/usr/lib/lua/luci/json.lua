...
-- @return Lua object
function decode(json, func, ...)
	local a = ActiveDecoder(function() return nil end, ...)
	a.chunk = json
	-- added by LiGuanghua, support decode url
	a.func = func
...
function Decoder.parse_null(self, chunk)
	...
	-- modified by LiGuanghua, parse json null to empty table
	return self:parse_literal(chunk, "null", {})
end
...
function Decoder.parse_string(self, chunk)
	...
	-- added by LiGuanghua, decode url
	if self["func"] then
		str = self["func"](str)
	end
