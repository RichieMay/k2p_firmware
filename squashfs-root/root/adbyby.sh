...
wget --no-check-certificate https://raw.githubusercontent.com/adbyby/Files/master/7620n.tar.gz -O /tmp/7620n.tar.gz -t 1 -T 5 2>/dev/null
if [ "$?" == "0" ]; then
wget_ok="1"
else
...
wget http://iytc.net/tools/kp.dat -O /tmp/koolproxy/data/rules/kp.dat -t 1 -T 5 2>/dev/null
fi
...
wget --no-check-certificate https://raw.githubusercontent.com/adbyby/xwhyc-rules/master/lazy.txt  -O /tmp/adbyby/bin/data/lazy.txt -t 1 -T 5 2>/dev/null
else
...
if [ "$get_source" == "0" ]; then
#wget $murl -O $mfile -t 1 -T 5 2>/dev/null
wget --no-check-certificate https://raw.githubusercontent.com/adbyby/xwhyc-rules/master/video.txt -O /tmp/adbyby/bin/data/video.txt -t 1 -T 5 2>/dev/null
else
...
if [ ! -f /tmp/adbyby_host.conf ] ;then
cp -f /etc/adbset/adblock.conf /tmp/dnsmasq.d/adbyby_host.conf
else
cp -f /tmp/adbyby_host.conf /tmp/dnsmasq.d/adbyby_host.conf
fi
sed -i 's/adblock/adbyby_list/g' /tmp/dnsmasq.d/adbyby_host.conf
sed -i '/google/d' /tmp/dnsmasq.d/adbyby_host.conf
sed -i '/youtube/d' /tmp/dnsmasq.d/adbyby_host.conf
